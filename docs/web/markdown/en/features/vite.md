...
$ npx ssr start --tools vite # 等价于 npm run start:vite
$ npx ssr build --tools vite # 等价于 npm run build:vite
...
* `package.json` 新增 `ssr build --tools vite` 相关脚本

```json
"scripts": {
  "start:vite": "ssr start --tools vite",
  "build:vite": "ssr build --tools vite"
}
```
...
* 同时支持 `Webpack/Vite` 两种方案，当使用 `ssr start --tools vite` 进行本地开发时使用 `Webpack` 去打包 服务端 `bundle`，然后在 Node.js 框架中加载 `Vite` 提供的中间件，让 `Vite` 服务接管客户端文件请求。
