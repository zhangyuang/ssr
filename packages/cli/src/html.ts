...
export const generateHtml = async () => {
  ...
  const combine = [
    {
      arr: getScriptArr(header, mockCtx).concat(
        getScriptArr(staticConfig.customeHeadScript ?? [], mockCtx),
      ),
      flag: "header",
    },
    {
      arr: getScriptArr(footer, mockCtx).concat(
        getScriptArr(staticConfig.customeFooterScript ?? [], mockCtx),
      ),
      flag: "footer",
    },
  ];
  ...
  if (framework === "ssr-plugin-react") {
    ...
    for (const item of combine) {
      ...
      if (flag === "header") {
        jsHeaderManifest += (await renderToString(h(Fragment, {}, scriptArr))).replace(
          'data-reactroot=""',
          "",
        );
      } else {
        jsFooterManifest += (await renderToString(h(Fragment, {}, scriptArr))).replace(
          'data-reactroot=""',
          "",
        );
      }
