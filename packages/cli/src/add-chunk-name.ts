...
export const addChunkNameInRoutes = async () => {
  ...
  const cwd = getCwd();
  const declareRoutes = await promises.readFile(
    resolve(cwd, "./build/ssr-declare-routes.js"),
    "utf-8",
  );
  await promises.writeFile(
    resolve(cwd, "./build/ssr-declare-routes.js"),
    addChunkNameInQuery(declareRoutes),
  );
  const manualRoutes = await promises.readFile(
    resolve(cwd, "./build/ssr-manual-routes.js"),
    "utf-8",
  );
  await promises.writeFile(
    resolve(cwd, "./build/ssr-manual-routes.js"),
    addChunkNameInQuery(manualRoutes),
  );
};
