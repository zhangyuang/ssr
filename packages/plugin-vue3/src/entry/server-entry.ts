...
const serverRender = async (ctx: ISSRContext, config: IConfig) => {
	...
	const getApp = ({ combineAysncData, state, layoutFetchData, asyncData, manifest, isCsr, jsInject, cssInject, inlineCssOrder, rootId, inlineJsOrder }: vue3AppParams) => {
		const app = createSSRApp({
			render: function () {
				...
				).concat(getInlineVNode(inlineCssOrder, 'style', isVite))
				...
				).concat(getInlineVNode(inlineJsOrder, 'script', isVite))
				return h(
					Layout,
					{
						ctx,
	...
	const fn = async () => {
		...
		const { inlineCssOrder, extraCssOrder, inlineJsOrder, extraJsOrder } = await getInlineOrder({
			dynamicCssOrder,
			dynamicJsOrder,
			manifest,
			config,
