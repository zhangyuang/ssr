...
export const checkRoute = ({ routeItem, path }: { routeItem?: { path: string }; path: string }) => {
  if (!routeItem?.path) {
    throw new Error(`
...
export const getStaticConfig = () => {
  ...
  const staticConfig = accessFileSync(staticConfigPath)
    ? (__non_webpack_require__(staticConfigPath) as UserConfig)
    : {};
  return staticConfig;
};
...
export const requireWithPreserveLinks = (path: string): string | undefined => {
  ...
  if (!accessFileSync(pkgJsonPath)) {
    logWarning(
      `Please verify the package.json file, current program use ${path} but don't specify it in dependencies`,
    );
    return;
  }
