...
const addAsyncChunk = async (webpackChunkName: string, config: IConfig, type: "css" | "js") => {
  ...
  for (const key in asyncChunkMap) {
    if (
      ...
      asyncChunkMap[key].includes(webpackChunkName + "-fetch")
    ) {
      arr.push(`${key}.${type}`);
    }
  }
...
export const getUserScriptVue = (options: {
  ...
  const staticScript =
    position === "header" ? staticConfig.customeHeadScript : staticConfig.customeFooterScript;
...
const getInlineContent = async (
  ...
  const inlineContent = await Promise.all(
    inlineOrder
      ...
      .map((item) =>
        promises.readFile(
          isAbsolute(item!) && !item!.startsWith("/client") ? item! : join(cwd, "./build", item!),
        ),
      ),
  );
