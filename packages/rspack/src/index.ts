...
export const start = async () => {
	...
	await Promise.all([startServerBuild(getServerRspack(serverConfigChain)), startClientServer(getClientRspack(clientConfigChain))])
}

export const build = async () => {
	const serverConfigChain = new RspackChain()
	const clientConfigChain = new RspackChain()
	await Promise.all([startServerBuild(getServerRspack(serverConfigChain)), startClientBuild(getClientRspack(clientConfigChain))])
}
