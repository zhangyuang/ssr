...
export default async ({ store, router }, ctx?: ISSRContext<{
  ...
  const data = __isBrowser__ ? await (await window.fetch(`/api/detail/${router.value ? router.value.params.id : router.params.id}`)).json() : await ctx?.apiDeatilservice?.index(ctx.request.params.id)
